...
std::string BridgeInstallScript(const std::set<std::string>& commands) {
  return "(function(){"
         ...
         "let lifecycleState='active';const lifecycleListeners=new Set();"
         ...
         "};"
         "window.__fenestraLifecycleSet=function(state,reason){"
         "lifecycleState=String(state||'active');const detail={state:lifecycleState,reason:String(reason||'')};"
         "for(const cb of Array.from(lifecycleListeners)){queueMicrotask(()=>cb(detail));}"
         "window.dispatchEvent(new CustomEvent('fenestra:lifecycle',{detail}));"
         "if(lifecycleState==='active'){window.dispatchEvent(new CustomEvent('fenestra:resume',{detail}));}"
         "else if(lifecycleState==='hibernate'){window.dispatchEvent(new CustomEvent('fenestra:hibernate',{detail}));}"
         "else{window.dispatchEvent(new CustomEvent('fenestra:suspend',{detail}));}"
         "};"
         ...
         "window.fenestra.lifecycle={get state(){return lifecycleState;},listen(callback){"
         "lifecycleListeners.add(callback);return()=>lifecycleListeners.delete(callback);},"
         "onSuspend(callback){return this.listen((event)=>{if(event.state==='suspended')callback(event);});},"
         "onResume(callback){return this.listen((event)=>{if(event.state==='active')callback(event);});},"
         "onHibernate(callback){return this.listen((event)=>{if(event.state==='hibernate')callback(event);});}};"
